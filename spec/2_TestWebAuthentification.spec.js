...
describe("Tester le service WebAuthentification", () => {
  describe("Tester la validation de la demande d'inscription d'un locataire", () => {
    ...
    beforeAll(async () => {
      await axios
        .post(
          url + "authentification_mobile/locataire_inscription/",
          data_insert
        )
        .then((res) => {
      ...
      await axios
        .put(
          url +
            "authentification_web/valider_demande/naoumiislend@gmail.com/demande/3"
        )
        .then((res) => {
          expect(res.status).toEqual(200);
        });
      ...
      await axios
        ...
        .then((res) => {
          ...
          res.data.forEach((element) => {
            if (
              element.email === "naoumiislend@gmail.com" &&
    ...
    it("le test passe si le compte du locataire a été validé", async () => {
      await axios
        .get(
          url +
            "authentification_mobile/locataire_connexion/naoumiislend@gmail.com"
        )
        .then((res) => {
          expect(res.status).toEqual(200);
  ...
  describe("Tester le refus de la demande d'inscription", () => {
    ...
    beforeAll(async () => {
      await axios
        .put(
          url +
            "authentification_web/refuser_demande/naoumiislend@gmail.com/demande/3",
          data_put
        )
        .then((res) => {
          expect(res.status).toEqual(200);
